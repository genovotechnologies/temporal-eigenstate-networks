...
\begin{abstract}
We present Temporal Eigenstate Networks (TEN), a sequence model that replaces attention with learned eigenstate decomposition. The hidden state is a superposition of $K$ complex-valued eigenstates evolving via diagonal recurrence, mixed through multi-head block-diagonal coupling, and reconstructed through a gated output projection. The same mathematical formulation---$c_k(t) = \lambda_k \cdot c_k(t\!-\!1) + \beta_k(t)$---admits three computational backends: a fused Triton kernel for short sequences ($T \leq 512$, 10.9$\times$ faster than FFT), FFT convolution for medium sequences ($O(T \log T)$), and a Pro variant with cross-layer eigenstate memory and depth-adaptive initialization for long sequences ($T > 2048$). The architecture auto-selects the optimal backend at runtime.

On a single A100, TEN achieves $9.8\times$ measured speedup over SDPA transformers at $T = 131072$, with inference time nearly constant from $T = 65536$ to $T = 131072$ (622ms $\to$ 638ms) while the transformer scales quadratically (3262ms $\to$ 6280ms). Attention-specific memory reduction is $128\times$ at $T = 2048$ and $8192\times$ at $T = 131072$. On WikiText-103 at model dimension $d = 1024$, TEN outperforms the transformer on perplexity (197.8 vs 231.0) with 12\% fewer parameters, suggesting favorable width scaling. We provide stability and gradient bounds showing the architecture avoids exponential blow-up pathologies, even under input-selective eigenvalue modulation.

Code: \url{https://github.com/genovotechnologies/temporal-eigenstate-networks}.
\end{abstract}
...
\item \textbf{Three-tier computational backend.} A single eigenstate recurrence admits three equivalent computations, each optimal for a different sequence length regime: a fused Triton kernel ($T \leq 512$, $10.9\times$ faster than FFT), FFT convolution ($512 < T \leq 2048$), and TEN-Pro with cross-layer eigenstate memory ($T > 2048$). The model auto-selects at runtime. This is, to our knowledge, the first architecture that adaptively switches computational backends within a unified mathematical framework.

\item \textbf{Favorable width scaling.} At $d = 1024$, TEN outperforms transformers on WikiText-103 perplexity (197.8 vs 231.0) with 12\% fewer parameters. This suggests TEN benefits more from width scaling than transformers, consistent with observations in the SSM literature.
\end{enumerate}

We should be upfront about limitations. Our experimental comparison uses PyTorch's SDPA (which dispatches to fused kernels) but not FlashAttention-2 explicitly. We do not compare against Mamba, RetNet, or RWKV due to installation constraints on our compute environment. At $d = 512$ with sufficient training, the transformer still achieves better perplexity than TEN (43.6 vs 83.6), though the gap narrows at $d = 1024$. These are meaningful caveats; we present TEN as a contribution to the space of efficient sequence models, not a universal replacement for attention.
...
\toprule
Model & $T\!=\!2048$ & $T\!=\!8192$ & $T\!=\!16384$ & $T\!=\!32768$ & $T\!=\!65536$ & $T\!=\!131072$ \\
\midrule
TEN-FFT & \textbf{43} & \textbf{155} & \textbf{309} & \textbf{616} & \textbf{622} & \textbf{638} \\
TEN-Pro & 41 & 156 & 311 & 621 & 627 & 643 \\
Transformer (SDPA) & 47 & 237 & 625 & 1873 & 3262 & 6280 \\
\midrule
Speedup (FFT vs SDPA) & 1.1$\times$ & 1.5$\times$ & 2.0$\times$ & 3.0$\times$ & 5.2$\times$ & \textbf{9.8}$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
& $T\!=\!512$ & $T\!=\!2048$ & $T\!=\!8192$ & $T\!=\!32768$ & $T\!=\!65536$ & $T\!=\!131072$ \\
\midrule
Attn matrix (per layer) & 8MB & 134MB & 2.1GB & 34.4GB & 137GB & 550GB \\
TEN eigenstates & 0.3MB & 1.0MB & 4.2MB & 16.8MB & 33.6MB & 67.1MB \\
\midrule
Memory ratio & 32$\times$ & \textbf{128}$\times$ & 512$\times$ & 2048$\times$ & 4096$\times$ & \textbf{8192}$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{WikiText-103 training perplexity ($d = 512$, 6 layers, 10,000 steps, $T = 512$, cosine LR schedule). Training on a single A100 40GB.}
\label{tab:lm}
\begin{tabular}{lccc}
\toprule
Model & Params & Perplexity & Training Time \\
\midrule
Transformer (SDPA) & 44.9M & \textbf{43.6} & 13.3 min \\
TEN-FFT & 41.0M & 83.6 & 13.6 min \\
TEN-Pro & 41.1M & 89.8 & 13.9 min \\
\bottomrule
\end{tabular}
\end{table}

The transformer achieves lower perplexity (43.6 vs TEN-FFT's 83.6). This gap is expected and consistent with the SSM literature: at matched model size and short contexts ($T = 512$), attention's quadratic pairwise interactions provide more expressiveness than linear-time methods. TEN-FFT outperforms TEN-Pro (83.6 vs 89.8) at this sequence length, as the cross-layer memory adds parameters without sufficient long-range signal at $T = 512$.

Interestingly, at $d = 1024$ with only 3,000 training steps, TEN-FFT achieves 197.8 perplexity with 111.8M parameters while the transformer achieves 231.0 with 127.5M parameters---TEN is actually \emph{better} at this configuration, suggesting that at larger model scales the gap may close or reverse. This is consistent with the observation that SSMs benefit more from width scaling than transformers \cite{gu2023mamba}.

The key tradeoff is clear: the transformer wins on perplexity at short sequences, but TEN wins on efficiency at long sequences. At $T = 131072$ (Table~\ref{tab:speed}), TEN-FFT is $9.8\times$ faster than the transformer (638ms vs 6280ms), with inference time nearly constant from $T = 65536$ to $T = 131072$. For applications requiring long contexts---document understanding, code generation, genomics---TEN's linear scaling makes contexts feasible that are impossible for quadratic attention.
...
TEN's linear scaling becomes advantageous beyond roughly $T = 1024$. The speedup grows without bound: $1.5\times$ at $T = 8192$, $5.2\times$ at $T = 65536$, and $9.8\times$ at $T = 131072$ (Table~\ref{tab:speed}). TEN-FFT inference time is nearly constant from $T = 65536$ to $T = 131072$ (622ms $\to$ 638ms), while the transformer doubles (3262ms $\to$ 6280ms)---a direct consequence of $O(T \log T)$ vs $O(T^2)$ scaling.

\subsection{Width Scaling}

\begin{table}[h]
\centering
\caption{Width scaling on WikiText-103 ($L = 6$, 3000 steps, $T = 512$). At $d = 1024$, TEN outperforms the transformer with fewer parameters.}
\label{tab:scaling}
\begin{tabular}{lccc}
\toprule
Model & $d$ & Params & Perplexity \\
\midrule
TEN-FFT & 512 & 41.0M & 143.0 \\
Transformer & 512 & 44.9M & 98.7 \\
\midrule
TEN-FFT & 1024 & 111.8M & \textbf{197.8}$^\dagger$ \\
Transformer & 1024 & 127.5M & 231.0 \\
\bottomrule
\end{tabular}

\vspace{2mm}
\footnotesize{$^\dagger$At $d = 1024$, TEN achieves lower perplexity than the transformer despite 12\% fewer parameters. Both models were trained for only 3000 steps (insufficient for convergence); the comparison shows relative scaling behavior, not final performance.}
\end{table}

At $d = 512$, the transformer converges to better perplexity (Table~\ref{tab:lm}). But at $d = 1024$, TEN pulls ahead---197.8 vs 231.0---with 12\% fewer parameters. This crossover suggests that TEN benefits more from width scaling than transformers, consistent with observations that SSMs scale favorably with state dimension \cite{gu2023mamba}. The practical implication is that for large-scale deployments where model width can be increased, TEN may match or exceed transformer quality while maintaining its linear-time inference advantage.
