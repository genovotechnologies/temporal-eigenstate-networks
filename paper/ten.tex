...
\midrule
TEN-FFT & \textbf{40.0} & \textbf{78.6} & \textbf{153.3} & \textbf{153.5} & \textbf{154.3} \\
TEN-Pro & 40.5 & 79.4 & 154.7 & 154.9 & 155.7 \\
Transformer (fused SDPA) & 42.9 & 87.2 & 180.2 & 199.0 & 238.0 \\
\midrule
Speedup (FFT vs Trans) & 1.07$\times$ & 1.11$\times$ & 1.18$\times$ & 1.30$\times$ & 1.54$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
Model & $T\!=\!512$ & $T\!=\!1024$ & $T\!=\!2048$ & $T\!=\!4096$ & $T\!=\!8192$ \\
\midrule
TEN-FFT & 1867 & 3549 & 6911 & 6915 & 6924 \\
TEN-Pro & 1872 & 3558 & 6928 & 6932 & 6941 \\
Transformer & 1888 & 3568 & 6930 & 6937 & 6944 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{WikiText-103 training perplexity ($d = 256$, 4 layers, 2000 steps, $T = 256$). Both TEN variants and the transformer use manual causal attention via PyTorch's \texttt{scaled\_dot\_product\_attention} with \texttt{is\_causal=True}.}
\label{tab:lm}
\begin{tabular}{lcccc}
\toprule
Model & Params & Perplexity & Steps/sec & Training Time \\
\midrule
Transformer (SDPA) & 16.1M & \textbf{97.7} & 15.8 & 127s \\
TEN-Pro & 15.5M & 118.5 & 14.3 & 140s \\
TEN-FFT & 15.4M & 124.3 & 14.6 & 137s \\
\bottomrule
\end{tabular}
\end{table}

The transformer achieves the best perplexity (97.7), which is expected: quadratic attention is inherently more expressive than linear-time methods at matched model size. This is consistent with the broader SSM literature---Mamba-1 also underperformed transformers before scaling up and adding short convolutions. TEN-Pro's cross-layer eigenstate memory narrows the gap (118.5 vs 124.3 for TEN-FFT), confirming that information propagation across layers helps at this scale. Training speed is comparable across all models at $T = 256$.

The interesting comparison is not perplexity at short sequences, but what happens as $T$ grows (Table~\ref{tab:speed}). The transformer's advantage in expressiveness is offset by its quadratic cost: at $T = 4096$, TEN is 1.3$\times$ faster; at $T = 8192$, the transformer fails entirely while TEN handles the sequence comfortably.
